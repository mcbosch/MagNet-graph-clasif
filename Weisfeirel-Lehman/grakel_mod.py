...
class WeisfeilerLehman(Kernel):
    ...
    def parse_input(self, X):
        ...
        """
        if self._method_calling not in [1, 2]: # Que significa això=
        ...
        def generate_graphs(label_count, WL_labels_inverse):
            breakpoint()
            new_graphs = list()
            # nx is the number of graphs
            #breakpoint()
            for j in range(nx):
                # Change the labels that we call with the first indexes.
                new_labels = dict()
                r"""L is a dictionary with keys graph index, associated
                to a dictionary with keys nodes and value ACTUAL label
                """
                for k in L[j].keys():
            ...
            for i in range(1, self._n_iter):
                # Repetim l'algoritme de WeisfeilerLehman _n_iter
                label_set, WL_labels_inverse, L_temp = set(), dict(), dict()
                for j in range(nx): # Assignam nous labels a
                    # Find unique labels and sort
                    # them for both graphs
                    # Keep for each node the temporary
                    L_temp[j] = dict()
                    r"""
                    Gs_ed és un diccionari que conté cada graf representat amb un diccionari,
                    on a cada node li assigna un altre diccionari, node adjacent -> label
                    """
                    for v in Gs_ed[j].keys():
                        # iteram els nodes del graf j-èssim
                        credential = str(L[j][v]) + "," + \
                            str(sorted([L[j][n] for n in Gs_ed[j][v].keys()])) # afegim atributs dels adjacents (ordenats)
                        L_temp[j][v] = credential # assignam al node el nou atribut a L_temp un nou diccionari
                ...
                yield new_graphs
            # Acaba funció generate graphs
        base_graph_kernel = {i: self._base_graph_kernel(**self._params) for i in range(self._n_iter)}
        if self._parallel is None:
            if self._method_calling == 1:
                print(list(enumerate(generate_graphs(label_count,WL_labels_inverse))))
                print(label_count)
    ...
    def transform(self, X):
        ...
        def generate_graphs(WL_labels_inverse, nl):
            ...
            for j in range(nx):
                ...
                new_graphs.append((Gs_ed[j], new_labels) + extras[j])
            print('new_graphs_from transform')
            return new_graphs 
